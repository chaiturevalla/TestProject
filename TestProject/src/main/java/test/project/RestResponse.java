...
/**
 * Author: Shanumuka R Version: 1.0 Date of Creation: 28/02/2022 Class
 * Description: This is Java bean class hold objects and also perform validation
 * using hamcrest.Matchers Built on top of Rest Assured
 */
public class RestResponse {
	...
	/**
	 * @return response return as pretty print format for eg., XML and Json
	 *         alignment
	 */
	...
	/**
	 * @Validateing JSON schema from response object
	 */
	public void validateJsonSchema(File schema) {
		response.then().log().all().assertThat().body(JsonSchemaValidator.matchesJsonSchema(schema));
	}
	 ...
	 * @return
	 */
	public RestResponse hamcrestResponseBodyValidation(String validationType, String elementName,
		...
		case "equalTo":
			...
			break;
		case "hasSize":
			response.then().log().all().body(elementName, hasSize(Integer.parseInt(expectedResults))).log()
					.ifValidationFails();
			break;
		case "hasItem":
			response.then().log().all().body(elementName, hasItem(Integer.parseInt(expectedResults))).log()
					.ifValidationFails();
			break;
		case "hasItemString":
	 ...
	 * @param elementName    element to be located All elements return string value,
	 *                       Need to typecast if required
	 * @return elementValue
	 */
	 ...
	 * @param expectedResults expected result for tests All elements return string
	 *                        value, Need to type case if required
	 */
	 ...
	 *
	 * @param jsonArrayName element to be searched and return in List with
	 *                      collections
	 */
	 ...
	 *
	 * @param jsonArrayName supply ArrayName to return as list with Key and value
	 *                      pair
	 */
	...
	/**
	 * To return response message as a list Can pass normal root.element as well as
	 * support groovy commands
	 *
	...
	/*
	 * To get multiple elements using Json Path method parameter can be passed like
	 * this "RestResponse.result.country" which returns country value
	 */
	...
	/*
	 * This will return value for specific element It supports both XML and Json
	 * value For Json: rootnode.node as well as groovy commands
	 */
