...
public class RestAssuredCore {
    ...
    public RestAssuredCore(String baseURI, String basePath) {
        ...
        request = RestAssured.given().filter(new AllureRestAssured()).log().all();
    }
    ...
    public static void initializeLogger() throws IOException {
        printLogFile = new PrintStream(new BufferedOutputStream(new FileOutputStream(new File(ConfigLoader.getInstance().getLogFilePath()), true)));
    ...
    * @param requestType supply keys like "GET","PUT","POST","DELETE","PATCH"
    * @param resourcePath is combination of baseURI, basePATH and resourcePATH
    * @param requestBody is the body required for the request
    * @return Instantiating Response class to hold response object after invoking request
    */
   public RestResponse invokeRestCall(String requestType, String resourcePath, String requestBody) {
       switch (requestType) {
           case "POST":
        	   response = request.log().all().contentType(ContentType.JSON).body(requestBody).post(resourcePath);
        	   System.out.println(response.getBody().asString());
               break;
           case "PUT":
        	   response = request.log().all().contentType(ContentType.JSON).body(requestBody).put(resourcePath);
        	   System.out.println(response.getBody().asString());
               break;
           default:
               break;
